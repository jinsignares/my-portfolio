...
function Projects({ projects }: Props) {
    ...
    return (
        <motion.div
            ...
            <div className=' flex overflow-x-scroll snap-mandatory snap-x z-20'>
                {projects.sort((a, b) => new Date(b.dateStarted).valueOf() - new Date(a.dateStarted).valueOf()).map((project, index) => (
                    <div className='flex w-screen flex-shrink-0 flex-col justify-start items-center snap-center space-y-5 md:p-44 px-3' key={index}>
                        <motion.div
                            initial={{
                                opacity: 0
                            }}
                        ...
                        <div className='space-y-6 md:px-10 max-w-[800px]'>
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="flex space-x-4 mt-4">
                                {project?.technologies.map((technology) => (
                                    <div className="flex flex-col justify-center items-center space-y-2" key={technology._id}>
                                        ...
                                        <p className="text-xs md:text-lg text-center md:text-left">{technology.title}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
